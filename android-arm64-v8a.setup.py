...
class PyBuildExt(build_ext):
    ...
    def build_extensions(self):
        ...
        extensions = [
            ext for ext in self.extensions if ext.name not in disabled_module_list
        ]
        ...
        for ext in self.extensions:
            ext.sources = [
                find_module_file(filename, moddirlist) for filename in ext.sources
            ]
        ...
        if any("_ssl" in l for l in (missing, self.failed, self.failed_on_import)):
            print()
    ...
    def build_extension(self, ext):
        ...
        try:
            ...
            self.announce(
                'WARNING: building of extension "%s" failed: %s'
    ...
    def check_extension_import(self, ext):
        ...
        if ext.name in self.failed:
            self.announce(
                'WARNING: skipping import check for failed build "%s"' % ext.name,
                level=1,
            )
            return
        ...
        if "Carbon" in ext.extra_link_args:
            self.announce(
                'WARNING: skipping import check for Carbon-based "%s"' % ext.name
            )
            return
        ...
        if host_platform == "cygwin":
            self.announce(
                'WARNING: skipping import check for Cygwin-based "%s"' % ext.name
            )
            return
        ...
        spec = importlib.util.spec_from_file_location(
            ext.name, ext_filename, loader=loader
        )
        try:
    ...
    def add_multiarch_paths(self):
        ...
        ret = os.system("%s -print-multiarch > %s 2> /dev/null" % (cc, tmpfile))
    ...
    def detect_modules(self):
        ...
        if cross_compiling:
            add_dir_to_list(
                self.compiler.include_dirs,
                "/opt/android/ndk/default/toolchains/llvm/prebuilt/linux-x86_64/sysroot/usr/include",
            )
            add_dir_to_list(
                self.compiler.library_dirs,
                "/opt/android/ndk/default/toolchains/llvm/prebuilt/linux-x86_64/sysroot/usr/lib/aarch64-linux-android/29/",
            )
            add_dir_to_list(
                self.compiler.library_dirs, "/data/data/com.twosix.race/race/python/"
            )
        ...
        if (
            ...
            add_dir_to_list(
                self.compiler.library_dirs, sysconfig.get_config_var("LIBDIR")
            )
            add_dir_to_list(
                self.compiler.include_dirs, sysconfig.get_config_var("INCLUDEDIR")
            )
        ...
        if not cross_compiling:
            ...
            # libraries.
            lib_dirs = self.compiler.library_dirs + sysroot_paths(
                ("LDFLAGS", "CC"), system_lib_dirs
            )
            inc_dirs = self.compiler.include_dirs + sysroot_paths(
        ...
        exts.append(Extension("time", ["timemodule.c"], libraries=time_libs))
        # libm is needed by delta_new() that uses round() and by accum() that
        # uses modf().
        exts.append(Extension("_datetime", ["_datetimemodule.c"], libraries=["m"]))
        ...
        exts.append(
            Extension("_testcapi", ["_testcapimodule.c"], depends=["testcapi_long.h"])
        )
        ...
        if config_h_vars.get("HAVE_GETSPNAM", False) or config_h_vars.get(
            "HAVE_GETSPENT", False
        ):
        ...
        exts.append(Extension("audioop", ["audioop.c"], libraries=["m"]))
        ...
        if do_readline:
            if cross_compiling:
                ret = os.system(
                    "%s -d %s | grep '(NEEDED)' > %s"
                    % (sysconfig.get_config_var("READELF"), do_readline, tmpfile)
                )
        ...
        if host_platform == "darwin":
            ...
            if dep_target and (
                tuple(int(n) for n in dep_target.split(".")[0:2]) < (10, 5)
            ):
                os_release = 8
            if os_release < 9:
        ...
        if do_readline:
            ...
            if readline_termcap_library:
                ...
                lib_dirs + ["/usr/lib/termcap"], "termcap"
            ):
        ...
        # socket(2)
        exts.append(
            Extension("_socket", ["socketmodule.c"], depends=["socketmodule.h"])
        )
        ...
        exts.append(Extension("_sha256", ["sha256module.c"], depends=["hashlib.h"]))
        exts.append(Extension("_sha512", ["sha512module.c"], depends=["hashlib.h"]))
        exts.append(Extension("_md5", ["md5module.c"], depends=["hashlib.h"]))
        exts.append(Extension("_sha1", ["sha1module.c"], depends=["hashlib.h"]))

        blake2_deps = glob(os.path.join(os.getcwd(), srcdir, "Modules/_blake2/impl/*"))
        ...
        sha3_deps = glob(os.path.join(os.getcwd(), srcdir, "Modules/_sha3/kcp/*"))
        sha3_deps.append("hashlib.h")
        exts.append(Extension("_sha3", ["_sha3/sha3module.c"], depends=sha3_deps))
            ...
            if os.path.exists(f):
                ...
                if m:
                    ...
                    sqlite_version_tuple = tuple(
                        [int(x) for x in sqlite_version.split(".")]
                    )
        ...
        if "gdbm" in dbm_order and self.compiler.find_library_file(lib_dirs, "gdbm"):
            exts.append(Extension("_gdbm", ["_gdbmmodule.c"], libraries=["gdbm"]))
        else:
        ...
        if module_enabled(exts, "_curses") and self.compiler.find_library_file(
            lib_dirs, panel_library
        ):
            exts.append(
                Extension(
                    "_curses_panel",
        ...
        if self.compiler.find_library_file(lib_dirs, "lzma"):
            exts.append(Extension("_lzma", ["_lzmamodule.c"], libraries=["lzma"]))
        else:
        ...
        if "--with-system-expat" in sysconfig.get_config_var("CONFIG_ARGS"):
            ...
            ret = os.system(
                '"%s" -Werror -Wno-unreachable-code -E -xc /dev/null >/dev/null 2>&1'
                % cc
            )
            if ret >> 8 == 0:
        ...
        exts.append(Extension("_multibytecodec", ["cjkcodecs/multibytecodec.c"]))
        for loc in ("kr", "jp", "cn", "tw", "hk", "iso2022"):
            exts.append(Extension("_codecs_%s" % loc, ["cjkcodecs/_codecs_%s.c" % loc]))
        ...
        if host_platform == "win32":
            ...
            if sysconfig.get_config_var(
                "HAVE_SEM_OPEN"
            ) and not sysconfig.get_config_var("POSIX_SEMAPHORES_NOT_ENABLED"):
    ...
    def detect_tkinter_darwin(self, inc_dirs, lib_dirs):
        ...
        if is_macosx_sdk_path(F):
            ...
            os.system(
                "file %s/Tk.framework/Tk | grep 'for architecture' > %s" % (F, tmpfile)
            )
    ...
    def detect_tkinter(self, inc_dirs, lib_dirs):
        ...
        if host_platform == "darwin" and self.detect_tkinter_darwin(inc_dirs, lib_dirs):
            return
        ...
        if (
            tcllib is None
            or tklib is None
            or tcl_includes is None
            or tk_includes is None
        ):
        ...
        if self.compiler.find_library_file(lib_dirs + added_lib_dirs, "BLT8.0"):
            ...
            libs.append("BLT8.0")
        elif self.compiler.find_library_file(lib_dirs + added_lib_dirs, "BLT"):
            defs.append(("WITH_BLT", 1))
            libs.append("BLT")
    ...
    def configure_ctypes_darwin(self, ext):
        ...
        srcdir = sysconfig.get_config_var("srcdir")
        ffi_srcdir = os.path.abspath(
            os.path.join(srcdir, "Modules", "_ctypes", "libffi_osx")
        )
        sources = [
        ...
        include_dirs = [
            os.path.join(ffi_srcdir, "include"),
    ...
    def detect_ctypes(self, inc_dirs, lib_dirs):
        ...
        ext = Extension(
            ...
            extra_compile_args=extra_compile_args,
            extra_link_args=extra_link_args,
            libraries=["/android/arm64-v8a/lib/libpython3.7m.so"],
            sources=sources,
            depends=depends,
        )
        # function my_sqrt() needs libm for sqrt()
        ext_test = Extension(
            "_ctypes_test", sources=["_ctypes/_ctypes_test.c"], libraries=["m"]
        )
        ...
        inc_dirs.append("/android/arm64-v8a/include")
        ffi_inc = ["/android/arm64-v8a/include"]
        ...
        ffi_lib = None
        lib_dirs.append("/android/arm64-v8a/lib")
        if ffi_inc is not None:
            for lib_name in ("ffi", "ffi_pic"):
                if self.compiler.find_library_file(lib_dirs, lib_name):
                    ffi_lib = lib_name
                    break
        ffi_lib = "/android/arm64-v8a/lib/libffi.so"
        ffi_h = "/android/arm64-v8a/include/ffi.h"
    ...
    def _decimal_ext(self):
        ...
        if "--with-system-libmpdec" in sysconfig.get_config_var("CONFIG_ARGS"):
            ...
            srcdir = sysconfig.get_config_var("srcdir")
            include_dirs = [
                os.path.abspath(os.path.join(srcdir, "Modules", "_decimal", "libmpdec"))
            ]
            libraries = ["m"]
            sources = [
                "_decimal/_decimal.c",
    ...
    def _detect_openssl(self, inc_dirs, lib_dirs):
        ...
        # RACE 1.1.1 These flags aren't being passed down for some reason
        # openssl_includes = split_var('OPENSSL_INCLUDES', '-I')
        # openssl_libdirs = split_var('OPENSSL_LDFLAGS', '-L')
        openssl_libs = split_var("OPENSSL_LIBS", "-l")
        openssl_includes = ["/android/arm64-v8a/include/"]
        openssl_libdirs = ["/android/arm64-v8a/lib/"]
        if not openssl_libs:
            # libssl and libcrypto not found
            return None, None

        # Find OpenSSL includes
        ssl_incs = find_file("openssl/ssl.h", inc_dirs, openssl_includes)
        if ssl_incs is None:
            return None, None

        # OpenSSL 1.0.2 uses Kerberos for KRB5 ciphers
        krb5_h = find_file("krb5.h", inc_dirs, ["/usr/kerberos/include"])
        if krb5_h:
            ssl_incs.extend(krb5_h)
        # RACE 1.1.1 These flags aren't being passed down for some reason
        config_vars["HAVE_X509_VERIFY_PARAM_SET1_HOST"] = True
